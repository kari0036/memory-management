...
public:
    ...
    explicit weak_ptr(const sh::shared_ptr<T>& r) noexcept;
    ...
    sh::shared_ptr<T> lock() const noexcept;
    void reset() noexcept;
...
template <typename T>
weak_ptr<T>::weak_ptr(const sh::shared_ptr<T>& r) noexcept
    : ptr_{r.ptr_}, controlBlock_{r.controlBlock_} {
    if (controlBlock_) {
        controlBlock_->incrementWeakRefs();
    }
...
template <typename T>
sh::shared_ptr<T> weak_ptr<T>::lock() const noexcept {
    return expired() ? sh::shared_ptr<T>() : sh::shared_ptr<T>(*this);
}
